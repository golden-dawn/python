...
Trade = recordclass("Trade", "stk, cp, exp, exp_bd, strike, opt_in, num, "
                    "avg_range, opt_out, opt_stats, opts, stp, ind_ranks")
IndRanks = recordclass("IndRanks", "r_rs_252, udv_21, udv_42, udv_63, rs_21, "
                       "min_rnk_252, min_rnk_252b")
...
def check_for_breaks(ts, jl, pivs, sgn):
    ...
    for ixx in range(-2, pivs_len, -2):
        ...
        prev_state = jl.last_rec('lns_s')
        # if dbg:
        #     print('{0:s}: max_px = {1:.2f}, px = {2:.2f} prev_dt = {3:s}, '
        #           'prev_lns={4:.2f}'.format(edt, max_px, px,
        #                                     jl.last_rec('dt', 2), prev_lns))
