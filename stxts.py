...
class StxTS:
    ...
    def find(self, dt, c = 0):
        ...
        if n < 0 :
            ...
            if c < 0: return self.l - 1
        else :
            df_dt = str(self.df.index[n].date())
            if df_dt == dt : return n
    ...
    def adjust(self, s_idx, e_idx, val, col_names, adj_type = 'split'):
        ratio = val if adj_type == "split" else (1 - val / self.df.loc[idx].c)
    ...
    def buckets(self, col_name, lb, ub, bucket, precision, suffix='b'):
        ...
        self.df[bucket_name] = self.df[col_name].map\
                               (lambda x: np.nan if np.isnan(x) else \
                                lb if x < lb else \
                                ub if x> ub else \
                                float(prec_str % (x / bucket)) * bucket)
    ...
    def addleader(self, tdf, liq='PctLiq', min_rank=1, max_rank=1000) :
        ...
        ldrfun1 = lambda x: 0 if(np.isnan(x['rank']) or x['rank']==0) else 1
        ...
        ldrfun2 = lambda x: -x['ldr'] if x['inv']==1 else x['ldr']
