...
class StxJL :
    ...
    def __init__(self, ts, f, w = 20) :
        ...
        self.last    = {'prim_px': 0, 'prim_state': StxJL.Nil, \
                        'px': 0, 'state': StxJL.Nil}
        self.ts.df['hb4l'] = self.ts.df.apply \
                             (lambda r: 1 if 2*r['c'] < r['h']+r['l'] else 0, \
                              axis=1)
    ...
    def jl(self, dt) :
        ...
        for ixx in range(start_w, end + 1):
            ...
            self.nextjl()
        return self.jl_recs
        # jldf    = pd.DataFrame(self.jl_recs[1:], columns=self.jl_recs[0])
        # jldf.set_index('dt', inplace='True')
        # return jldf
